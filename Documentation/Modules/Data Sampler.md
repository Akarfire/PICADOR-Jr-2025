...
* `samplePartcileDensity`- флаг сбора информации о плотности распределения частиц;
* `particleDensitySamplingParameters`- параметры сетки, используемой для сбора информации о плотности распределения частиц;
...
* `fieldSamplingParameters`- параметры сетки, используемой для сбора информации об электромагнитном поле;
...
Параметры сеток, используемых для сбора информации о плотности частиц и электромагнитном поле задаются при помощи структуры `GridSamplingParameters`:

```c++
struct GridSamplingParameters
{
	Vector3 samplingOrigin = Vector3::Zero; // Начало координат сетки

	size_t samplingResolutionX = 1; // Кол-во узлов сетки по оси X
	size_t samplingResolutionY = 1; // Кол-во узлов сетки по оси Y

	double samplingStepX = 0.1; // Пространственный шаг между узлами по оси X
	double samplingStepY = 0.1; // Пространственный шаг между узлами по оси Y
};
```

Сбор данных происходит в каждом узле указанной сетки.
